"""Export sealed PMCR-O trails to canonical SFT JSONL."""
...
def rows(trail:Path):
    tid=json.loads((trail/'trail.json').read_text(encoding='utf-8')).get('trailId',trail.name)
    cs=cycles(trail); ds=dispositions(trail); out=[]
    for c in sorted(cs):
        f=cs[c]; quality='pass' if admitted(f,ds.get(c)) else 'fail'
        for role in ORDER:
            if role not in f: continue
            frame=f[role]
            assistant={k:frame.get(k) for k in ('action','evidence','status')}
...
def export(trails:Path,out:Path,pass_only=True):
    ...
    with out.open('w',encoding='utf-8') as fh:
        for trail in sorted(trails.iterdir()):
            ...
            rs=rows(trail)
            findings=[f'{r["id"]}:{x.code}' for r in rs for x in validate_checker_row(r)]
