    ...
    ILogger<PmcroCycleSkill> logger) : AgentClassSkill<PmcroCycleSkill>
{
    ...
    public async Task<string> RunPmcroCycleAsync(
        ...
        [Description("Optional caller-supplied trail id for correlation. If omitted, a new correlation id is generated.")] string? trailId = null)
    {
        ...
        var executionSubject = subjectAgent;
        if (registry.Resolve(executionSubject) is null)
        {
            if (!ChiefDomains.Contains(subjectAgent))
                throw new InvalidOperationException(
                    $"No AIAgent registered for subjectAgent='{subjectAgent}'. Register it before dispatching a cycle.");

            executionSubject = "codeact-agent";
            if (registry.Resolve(executionSubject) is null)
                throw new InvalidOperationException(
                    $"Chief domain '{subjectAgent}' has no registered fallback 'codeact-agent'.");
        }

        // The declarative runner owns the authoritative execution graph. Its
        // internal trail id is currently generated by the runner; trailId remains
        // the caller correlation id in this compatibility surface until the runner
        // accepts externally supplied trail identities.
        var result = await declarativeRunner.RunAsync(seedIntent, executionSubject, project);
